...
| ConfigMap-backed Kubernetes repository | `repository.yaml.kubernetesRepositories[].repositories[].disabledPackages` | Hot reloaded after ConfigMap changes. |
...
Add `disabledPackages` to the repository ConfigMap. The same nested field is
valid under `kubernetesRepositories[].repositories[]`.
