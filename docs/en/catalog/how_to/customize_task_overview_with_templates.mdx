...
> Template rendering is an **alternative** to the `overview-markdown` result. If a `Task` still outputs a result named `overview-markdown`, the UI will **prefer that Markdown** and skip template rendering.
